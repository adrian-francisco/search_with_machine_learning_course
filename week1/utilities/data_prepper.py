    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        if response and response['hits']['hits'] and len(response['hits']['hits']) > 0:
            ...
            for hit in hits:
                ...
                feature_results["sku"].append(hit['_source']['sku'][0]) # Can there be multiple 'sku' values? Hard-coded only one.

                features = hit['fields']['_ltrlog'][0]['log_entry'] # Can there be multiple 'log_entry' values? Hard-coded only one.
